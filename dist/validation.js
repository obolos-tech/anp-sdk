...
export function validateDocument(doc) {
    ...
    if (doc.type === 'listing') {
        const data = doc.data;
        if (!data.title?.trim())
            return { valid: false, error: 'Listing title required' };
        if (!data.description?.trim())
            return { valid: false, error: 'Listing description required' };
        if (!data.minBudget)
            return { valid: false, error: 'minBudget required' };
        if (!data.maxBudget)
            return { valid: false, error: 'maxBudget required' };
    ...
    if (doc.type === 'bid') {
        const data = doc.data;
        if (!data.listingCid)
            return { valid: false, error: 'listingCid required' };
        if (!data.listingHash)
            return { valid: false, error: 'listingHash required' };
        if (!data.price)
    ...
    if (doc.type === 'acceptance') {
        ...
        if (!data.listingCid)
            return { valid: false, error: 'listingCid required' };
        if (!data.bidCid)
            return { valid: false, error: 'bidCid required' };
        if (!data.listingHash)
            return { valid: false, error: 'listingHash required' };
        if (!data.bidHash)
            return { valid: false, error: 'bidHash required' };
    }
    return { valid: true };
}
